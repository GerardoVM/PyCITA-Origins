...
baseMap.add_layer(
    ipyleaflet.TileLayer(url=GetTileLayerUrl(
        landsat_AWEI.mosaic().Or(landsat.mosaic())
    )
))
...
left_layer = ipyleaflet.TileLayer(url=GetTileLayerUrl(
        landsat_cloud_masked_median_AWEI.visualize(min=0.0, max=0.12, palette=["FFFFFF", "000000"], opacity= 1.0).subtract(landsat_cloud_masked_median_AWEI)
    ))
